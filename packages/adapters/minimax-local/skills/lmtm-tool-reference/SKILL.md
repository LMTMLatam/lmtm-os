...
- `lmtmGetCadenaPublicacion` — **la tool para saber si los clientes están publicando de verdad.** Mide por EFECTO (destino en Make, despacho real, posts en la red), no por etiquetas ni corridas en verde. Devuelve por cliente el PRIMER eslabón roto + un `detalle` con qué hacer y dónde. Reportá un solo eslabón por cliente, el primero: si no hay destino, que la red esté muda es consecuencia. `sync_ciego` = no lo estamos viendo, NO es que no publique. Si la tool falla diciendo que no se pudo leer Make, decí "no verificado", nunca "todo bien".
- `lmtmGetClientScheduledContent` — contenido programado en la lista de Redes de ClickUp. **`overdue=true` (el `startDate` ya pasó y `sentToMake=false`) = el webhook nunca disparó: eso SÍ se reporta.** Lo que NO vale es el converso: `overdue=false` NO significa que se publicó — la etiqueta "mandado a make" la pone ClickUp al llegar la fecha, antes de que Make haga nada, y queda puesta igual si el post se descartó. Para publicación real, `lmtmGetCadenaPublicacion` o `lmtmGetClientOrganicPosts`. Tampoco cuentan el status de ClickUp ("en curso", etc.) ni `plannedDate`/due_date, y las tareas SIN `startDate` son ideas sin programar — nunca reportarlas como atrasadas.
