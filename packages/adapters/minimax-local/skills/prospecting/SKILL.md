...
## User preferences for Automiq sessions
- Idioma y tono: Español (Argentina). Usar "vos" y mensajes concisos. Priorizar viñetas y respuestas accionables.
- Formato de entrega: artefacto principal en Markdown legible (.md). CSV sólo si el usuario lo pide explícitamente o la lista >25.
- Salida principal para LeadHunter: data/leadhunter-report-YYYY-MM-DD.md (MD legible). No incluir claves/secretos en los reports — reemplazar por [REDACTED].

## Leadhunter integration (session-driven additions)
- Propósito: extender la skill prospecting para integrarse directamente con el agente LeadHunter del repo `agency-agents-render`.
- Comportamiento recomendado del agente LeadHunter al usar esta skill:
  1. Ejecutar discovery extendido con browser-only (Google, webs de agencias, LinkedIn público, sitios de clientes de otras agencias) para identificar dolores solucionables.
  2. Aplicar checklist de "web analysis" (wa.me, booking, forms, chat widgets, listados recientes). Cada señal suma puntaje y debe guardarse con evidence_url + timestamp.
  3. Requerir contacto_verified=true para que un lead cuente como válido en el set de 10 diarios; si no hay verificación, reemplazar automáticamente.
  4. Cuando haya credenciales en entorno (APOLLO_API_KEY, ZOOMINFO_API_KEY, TRUELIST_API_KEY, CLAY_API_KEY), ejecutar enriquecimiento adicional y registrar consumo de créditos en el audit_trail. Antes de consumir créditos pedir confirmación explícita.
  5. Para entrega: producir el archivo Markdown en `data/leadhunter-report-YYYY-MM-DD.md` y opcionalmente `data/leadhunter-leads-YYYY-MM-DD.md` con la tabla resumida.
  6. Uso de plantillas: delegar generación de outreach_template a la skill `sales-agency-outbound` cuando esté disponible.
- Guardas de seguridad y cumplimiento:
  - No hacer scraping masivo ni bypass de CAPTCHAs.
  - Registrar siempre source URL + fecha por cada dato recopilado (GDPR / CAN-SPAM lineage).
  - No exponer keys en reportes; mostrar [REDACTED] en outputs o logs públicos.

## Quick checklist (Leadhunter)
- Objetivo diario: EXACTAMENTE 10 leads ofertables (fit_score 4–6), decisor identificado, contacto verificado (wa.me o teléfono móvil con evidence).
- Priorizar investigación en: clientes de otras agencias, posts LinkedIn con problema/crecimiento, ofertas de empleo relacionadas con ventas/operaciones, landing pages con booking/wa.me.
- Artefacto final: .md legible con tabla resumen + detalle por lead + evidencia (URLs).
