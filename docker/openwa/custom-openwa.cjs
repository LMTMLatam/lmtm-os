...
async function startWaAutomate() {
  try {
    ...
    try {
      ...
      const patchedLine = `// PATCHED by custom-openwa: extract QR first, then wait forever for Debug.VERSION\n            yield (async () => { let __emitted=false; let __tries=0; while(!__emitted && __tries<60){ __tries++; try{ const __info=await waPage.evaluate(\`(()=>{ try{ const c=document.querySelector('canvas[aria-label]') || document.querySelector('canvas'); if(!c){ const txt=(document.body && document.body.innerText || '').slice(0,300); return {qr:null,png:null,reason:'no-canvas | url='+location.href+' | title='+document.title+' | body='+txt}; } const ref=c.parentElement&&c.parentElement.getAttribute('data-ref'); const png=window.getQrPng ? window.getQrPng() : null; return {qr:ref,png:png,reason:'ok'}; }catch(e){ return {qr:null,png:null,reason:'err:'+e.message}; } })()\`); if(__info && (__info.qr || __info.png)){ const __ev=require('./events').ev; if(__info.qr) __ev.emit('qrData.'+config.sessionId, __info.qr, config.sessionId, 'qrData'); if(__info.png) __ev.emit('qr.'+config.sessionId, __info.png, config.sessionId, 'qr'); __emitted=true; break; } if(__tries===1 || __tries===5 || __tries===15 || __tries===30 || __tries===60) console.log('[patch] QR extract attempt '+__tries+': '+__info.reason); }catch(e){ console.log('[patch] QR extract err: '+e.message); } await new Promise(r=>setTimeout(r,1000)); } if(!__emitted) console.log('[patch] QR extract gave up after '+__tries+' attempts'); })();\n            yield waPage.waitForFunction('window.Debug!=undefined && window.Debug.VERSION!=undefined && require', { timeout: 0 });`;
